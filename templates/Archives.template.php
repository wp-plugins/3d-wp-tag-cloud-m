		...
		<div style="float: left;">
			...
			<div style="padding-top: 5px;">
				<label style="margin: 0 5px 0 0;" title="The shape of the cloud" for="<?=$this->get_field_id('arch_shape'); ?>">
					...
					Shape
					<br>
					<select id="<?=$this->get_field_id('arch_shape'); ?>" name="<?=$this->get_field_name('arch_shape'); ?>" onchange="check43d(this.value, '<?= $this->get_field_id('arch_radius_z'); ?>')";>
						...
						<option value="spiral" <?php if( $arch_shape == "spiral" ){ echo ' selected'; } ?>>spiral</option>
						<option value="hexagon" <?php if( $arch_shape == "hexagon" ){ echo ' selected'; } ?>>hexagon</option>						
					</select>
				</label>
			...
			<div style="padding-top: 5px;">
				...
				<label style="width: 86px;" title="Initial size of cloud from centre to sides." for="<?=$this->get_field_id('arch_radius_x'); ?>">
					...
					<select id="<?=$this->get_field_id('arch_radius_x'); ?>" name="<?=$this->get_field_name('arch_radius_x'); ?>">
						<?php for($i=0; $i<1005; $i+=5){echo '<option id="arx_' . $i . '" value="' . $i/100 . '"'; if($arch_radius_x==$i/100){echo ' selected';}; echo '>' . $i/100 . '</option>'; } ?>
					</select>
				</label>				
				<label style="width: 86px;" title="Initial size of cloud from centre to top and bottom." for="<?=$this->get_field_id('arch_radius_y'); ?>">
					...
					<select id="<?=$this->get_field_id('arch_radius_y'); ?>" name="<?=$this->get_field_name('arch_radius_y'); ?>">
						<?php for($i=0; $i<1005; $i+=5){echo '<option id="ary_' . $i . '" value="' . $i/100 . '"'; if($arch_radius_y==$i/100){echo ' selected';}; echo '>' . $i/100 . '</option>'; } ?>
					</select>
				</label>				
				<div style="width: 86px; float: left;<?php if($arch_shape == 'spiral'||$arch_shape == 'hexagon') {echo ' visibility: hidden;';}; ?>" title="Initial size of cloud from centre to front and back." id="cont_<?=$this->get_field_id('arch_radius_z'); ?>" <?php if($arch_shape == 'spiral'||$arch_shape == 'hexagon') {echo '; visibility: hidden;';}; ?>>
					Radius Z
					<br>
					<select id="<?=$this->get_field_id('arch_radius_z'); ?>" name="<?=$this->get_field_name('arch_radius_z'); ?>">
						<?php for($i=0; $i<1005; $i+=5){echo '<option id="arz_' . $i . '" value="' . $i/100 . '"'; if($arch_radius_z==$i/100){echo ' selected';}; echo '>' . $i/100 . '</option>'; } ?>
					</select>
				</div> 
			...
			<div>
				<div style="margin: 4px 37px 0 0; float: left; padding: 0 1px 1px 1px; border: 1px dotted #aaa; border-radius: 5px;" title="Starting rotation speed, with horizontal and vertical values as an array, e.g. <span class='green'>[0.5,-0.3]</span>. Values are multiplied by <span class='green'>maxSpeed</span>.">
					...
					<select id="<?=$this->get_field_id('arch_initial_x'); ?>" name="<?=$this->get_field_name('arch_initial_x'); ?>">
						<?php for($i=-100; $i<101; $i++){echo '<option id="ainx_' . $i . '" value="' . $i/100 . '"'; if($arch_initial_x==$i/100){echo ' selected';}; echo '>' . $i/100 . '</option>'; } ?>	
					</select><select id="<?=$this->get_field_id('arch_initial_y'); ?>" name="<?=$this->get_field_name('arch_initial_y'); ?>">
						<?php for($i=-100; $i<101; $i++){echo '<option id="ainy_' . $i . '" value="' . $i/100 . '"'; if($arch_initial_y==$i/100){echo ' selected';}; echo '>' . $i/100 . '</option>'; } ?>
					</select>
				</div>
